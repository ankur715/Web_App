# app.py - Skypad Analytics Chatbot (SQLite version, no BigQuery billing needed)
#
...
app = FastAPI(title="Skypad Analytics Chatbot")
...
# ---------------------------------------------------------------------------
DB_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "skypad_analytics.db")
...
def init_database():
    ...
    c.execute('''
        CREATE TABLE IF NOT EXISTS skypad_products (
    ...
    c.execute('''
        CREATE TABLE IF NOT EXISTS skypad_stores (
    ...
    c.execute('''
        CREATE TABLE IF NOT EXISTS skypad_sales_data (
    ...
    c.execute('''
        CREATE TABLE IF NOT EXISTS skypad_inventory (
    ...
    c.execute('''
        CREATE TABLE IF NOT EXISTS skypad_customers (
    ...
    c.execute("SELECT COUNT(*) FROM skypad_products")
    if c.fetchone()[0] == 0:
        c.executemany("INSERT INTO skypad_products VALUES (?, ?, ?, ?, ?)", PRODUCTS)
        c.executemany("INSERT INTO skypad_stores VALUES (?, ?, ?, ?, ?)", STORES)
        c.executemany("INSERT INTO skypad_customers VALUES (?, ?, ?, ?, ?)", CUSTOMERS)
        c.executemany(
            "INSERT INTO skypad_sales_data VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        ...
        for store in STORES:
            for product in rng.sample(PRODUCTS, k=6):
                ...
                inv_num += 1
        c.executemany(
            "INSERT INTO skypad_inventory VALUES (?, ?, ?, ?, ?)", inventory_rows
        )
...
# ---------------------------------------------------------------------------
SCHEMA_DESCRIPTION = """
1. skypad_products (product_id, product_name, category, brand, unit_price)
2. skypad_sales_data (transaction_id, store_id, product_id, customer_id, quantity,
...
3. skypad_inventory (inventory_id, store_id, product_id, quantity_on_hand, reorder_level)
4. skypad_stores (store_id, store_name, city, state, zip_code)
5. skypad_customers (customer_id, customer_name, email, loyalty_status, lifetime_value)
"""
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 60)
    print("🚀 Skypad Analytics Chatbot - SQLite Version")
    print("=" * 60)
...
# # app.py - SQLite Version (No BigQuery Billing Needed)
...
# except Exception as e:
#     print(f"❌ Google AI initialization error: {e}")
#     raise

# # SQLite Database Setup
# DB_PATH = 'skypad_analytics.db'
...
#             ('LV_001', 'Louis Vuitton Speedy', 'Bags', 'Louis Vuitton', 1450.00),
#         ]
#         c.executemany('INSERT INTO skypad_products VALUES (?, ?, ?, ?, ?)', products_data)
...
#             ('STORE_005', 'SAKS Fifth Avenue - Boston', 'Boston', 'MA', '02116'),
#         ]
#         c.executemany('INSERT INTO skypad_stores VALUES (?, ?, ?, ?, ?)', stores_data)
        
#         sales_data = [
#             ('TXN_001', 'STORE_001', 'NIKE_001', 'CUST_001', 2, 129.99, 259.98, '2024-11-01', '2024-11', 2024),
#             ('TXN_002', 'STORE_001', 'NIKE_002', 'CUST_002', 1, 139.99, 139.99, '2024-11-05', '2024-11', 2024),
#             ('TXN_003', 'STORE_002', 'NIKE_001', 'CUST_003', 3, 129.99, 389.97, '2024-11-10', '2024-11', 2024),
#             ('TXN_004', 'STORE_001', 'GUCCI_001', 'CUST_004', 1, 1290.00, 1290.00, '2024-11-12', '2024-11', 2024),
#             ('TXN_005', 'STORE_003', 'NIKE_003', 'CUST_005', 2, 109.99, 219.98, '2024-11-15', '2024-11', 2024),
#             ('TXN_006', 'STORE_001', 'NIKE_001', 'CUST_006', 1, 129.99, 129.99, '2024-11-18', '2024-11', 2024),
#             ('TXN_007', 'STORE_002', 'GUCCI_002', 'CUST_007', 1, 1150.00, 1150.00, '2024-11-20', '2024-11', 2024),
#             ('TXN_008', 'STORE_004', 'NIKE_002', 'CUST_008', 2, 139.99, 279.98, '2024-11-22', '2024-11', 2024),
#             ('TXN_009', 'STORE_001', 'COACH_001', 'CUST_009', 1, 295.00, 295.00, '2024-11-25', '2024-11', 2024),
#             ('TXN_010', 'STORE_005', 'NIKE_001', 'CUST_010', 4, 129.99, 519.96, '2024-11-28', '2024-11', 2024),
#         ]
#         c.executemany('INSERT INTO skypad_sales_data VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', sales_data)
        
#         inventory_data = [
#             ('INV_001', 'STORE_001', 'NIKE_001', 45, 20),
#             ('INV_002', 'STORE_001', 'NIKE_002', 32, 20),
#             ('INV_003', 'STORE_001', 'NIKE_003', 28, 15),
#             ('INV_004', 'STORE_001', 'GUCCI_001', 12, 5),
#             ('INV_005', 'STORE_001', 'COACH_001', 35, 15),
#             ('INV_006', 'STORE_002', 'NIKE_001', 55, 20),
#             ('INV_007', 'STORE_002', 'GUCCI_002', 8, 5),
#             ('INV_008', 'STORE_003', 'NIKE_003', 40, 15),
#         ]
#         c.executemany('INSERT INTO skypad_inventory VALUES (?, ?, ?, ?, ?)', inventory_data)
...
#             ('CUST_005', 'David Lee', 'david@email.com', 'Gold', 7654.80),
#         ]
#         c.executemany('INSERT INTO skypad_customers VALUES (?, ?, ?, ?, ?)', customers_data)
...
#     prompt = f"""
# You are a SQLite SQL expert. Convert this business question to SQL.
...
#     if not sql_result['success']:
#         return f"Error querying data: {sql_result['error']}"
...
#     data_text = json.dumps(rows, indent=2)
    
#     prompt = f"""
# You are a retail analytics expert. Summarize this data in a clear, concise way.
...
# Write a 2-3 sentence summary that directly answers the question:
# """
...
# @app.get("/health")
# async def health():
#     return {"status": "ok", "database": "SQLite", "db_file": DB_PATH}
...
#     print("\n📊 Backend running at: http://localhost:8000")
#     print("💬 API docs at: http://localhost:8000/docs")
#     print("🗄️  Database: skypad_analytics.db")
#     print("\n✅ Next: Start frontend in separate terminal")
#     print("   cd skypad-chatbot && npm start")
...
# # PROJECT_ID = os.getenv('GOOGLE_PROJECT_ID', 'test')
# # DATASET_ID = os.getenv('BIGQUERY_DATASET', 'skypad_analytics')
...
# #     genai.configure(api_key=os.getenv('GOOGLE_API_KEY'))
# #     llm = genai.GenerativeModel('gemini-pro')
# #     print(f"✓ Google Generative AI initialized")
# # except Exception as e:
# #     print(f"❌ Google AI initialization error: {e}")
# #     raise
